...
| `numerical` | 数值线性代数分解 | `lu`/`qr`/`eig`/`svd`/`solve`（nalgebra f64） |
| `time` | 时间计算域 | `date` / `datetime` / `timestamp` 等 14 函数（jiff 0.2 + IANA tzdb） |
| `unit` | 物理单位换算 | `convert(value, "from", "to")` 8 量纲 + 温度仿射 |
| `fx` | 汇率换算 | `fx(value, "FROM", "TO")`（frankfurter.dev API + 三级缓存） |
...
# （如需全 BigRational 精度，使用 precision(50, 1/3) 获得 0.33333...33 精确值）
```
