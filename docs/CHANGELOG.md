...
基于 codenexus 0.3.5 代码知识图谱分析的三大方向优化：循环依赖修复 + 复杂度热点重构 + 模块层次文档化。
详见 [ARCHITECTURE.md](ARCHITECTURE.md)。

#### 循环依赖修复（Phase 1-3）

- **evaluator 移至 core**（T001-T004）：`src/evaluator.rs` → `src/core/evaluator.rs`，`pub use` 路径调整，1793 测试通过
- **symbolic 移至 domains**（T005-T008）：`src/symbolic.rs` → `src/domains/symbolic.rs`，`pub mod domains` 开放，1793 测试通过
- **core ↔ domains 解耦**（T009-T012）：`build_default_router()` 从 `core/evaluator.rs` 迁至 `domains/factory.rs`；
  新增 `build_precision_domain()` 工厂函数消除 evaluator 对 `PrecisionDomain` 类型的直接依赖；
  类型依赖 11→0（-100%）、总依赖 12→2（-83%）、循环依赖消除 ✓

#### 复杂度热点重构（Phase 4-8）

5 个圈复杂度热点（cyc > 15）按 TDD 流程重构，主函数 + 提取方法全部 cyc ≤ 15：

| 文件 | 函数 | cyc 前 | cyc 后 | 降幅 | 提取方法 |
|------|------|--------|--------|------|----------|
| `domains/combinatorics.rs` | `eval_function` | 38 | 4 | -89% | `eval_permutation` / `eval_combination` / `eval_catalan` / `eval_stirling` + 2 helper |
| `domains/matrix.rs` | `eval_binary` | 25 | 4 | -84% | `eval_scalar_binary` / `eval_matrix_add_sub` / `eval_matrix_mul` / `eval_matrix_div` |
| `core/evaluator.rs` | `evaluate` | 25 | 7 | -72% | `validate_inputs` / `build_cache_key` / `eval_precision_mode` / `eval_regular_mode` |
| `core/canonicalizer.rs` | `transform_inner` | 24 | 2 | -92% | `transform_binary` / `transform_unary` / `transform_function` / `transform_matrix` / `transform_list` |
| `domains/symbolic.rs` | `eval_symbolic` | 23 | 15 | -35% | `eval_div`（除零 → ±Inf）/ `eval_ln`（非正 → NaN） |

#### 文档化（Phase 9）

- 新增 `docs/ARCHITECTURE.md`：四层分层架构（L1 核心 → L2 域 → L3 编排 → L4 入口）、循环依赖修复策略、复杂度治理结果、测试/基准结构设计预期

#### Phase 10 三维度审查修复

并行派遣 tiangang（安全）/ diting（架构+性能）/ kueiku（bug 分析）四维度审查，修复所有 HIGH 问题：

- **架构 HIGH-1**：`core/domain.rs` 测试代码直接构造 4 个具体域类型（违反 "core → domains 类型依赖 = 0" 声明）→ 迁移 priority 测试至 `domains/factory.rs` 测试模块，新增 `test_full_priority_table` 完整 priority 回归测试
- **架构 MEDIUM-1**：`ARCHITECTURE.md` 表 1.2 中 5 个域 priority 错误（StatisticsDomain 30→20、PolynomialDomain 30→25、NumberTheoryDomain 30→25、CombinatoricsDomain 30→25、PrecisionDomain 40→25）→ 修正为实际值并按 priority 升序排列
- **kueiku LOW**：`symbolic.rs:840` 注释事实错误（声称 0/0→NaN，实际 0.0.signum()=1.0 故 0/0→+Inf）→ 修正注释

审查结论：安全 0 CRITICAL/0 HIGH ✓ | 架构 0 CRITICAL/1 HIGH（已修复）✓ | 性能 0 CRITICAL/0 HIGH（21 benchmarks 0 回归）✓ | bug 0 CRITICAL/0 HIGH ✓

#### 验证

- `cargo test --features cli,server`：**1819 passed / 0 failed / 3 ignored**
- `cargo clippy --features cli,server --all-targets -- -D warnings`：0 告警
- `cargo fmt --all -- --check`：通过
- codenexus complexity 复测：5 个重构函数 + 21 个提取方法全部 cyc ≤ 15 ✓
- `grep -rn "crate::domains::.*Domain" src/core/`：0 匹配（core → domains 类型依赖 = 0，含测试代码）✓

#### commits

- `318f832` refactor(core,domains): break circular dep + rule 25 compliance (P2 Phase 1-3)
- `02af653` refactor(domains): combinatorics eval_function dispatch table (P2 Phase 4)
- `4ac18e8` refactor(matrix): extract eval_binary dispatch methods (cyc=25→4)
- `14d298d` refactor(evaluator): split evaluate into 4 single-responsibility functions (cyc=25→7)
- `b50f944` refactor(canonicalizer): extract transform_inner per-variant methods (cyc=24→2)
- `4c5d95a` refactor(symbolic): extract eval_div/eval_ln from eval_symbolic (cyc=23→15)
- `3db07d3` docs: add ARCHITECTURE.md + P2 changelog entry (P2 Phase 9)
- (Phase 10) fix(core,domains,docs): Phase 10 review fixes (priority test migration + ARCHITECTURE.md corrections + symbolic comment fix)
